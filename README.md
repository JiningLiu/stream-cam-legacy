...
echo "alias update='stop && cd ~/stream-cam-web/ && git reset --hard && git pull && npm i && cd ~/stream-cam/ && git reset --hard && git pull && cd server && cp mediamtx.yml ~/mediamtx.yml && npm i'" >> ~/.bashrc
source ~/.bashrc
