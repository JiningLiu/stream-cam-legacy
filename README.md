...
echo "alias update='stop && cd ~/stream-cam-web/ && git reset --hard && git pull && npm i && cd ~/stream-cam/ && git reset --hard && git pull && cp mediamtx.yml ~/mediamtx.yml && cd server && npm i'" >> ~/.bashrc
source ~/.bashrc
